    ...
    <div className="main-page" id='home'>
      <header className='text-head'>
        ...
        <div className='main-page-fun'>
          <p>-Το εργαστήρι μας για εκτύπωση 3D στην Κύπρο είναι εδώ για να εκτυπώσει οτιδήποτε έχετε στο μυαλό σας.</p>
          <div className='two-things'>
            <p>-Οποιεσδήποτε ιδέες;</p>
            <strong>-Όλες.</strong>
          </div>
        </div>
      </header>
      <div className="imgBox">
        {images.map((image, index) => (
          <img
            ...
            src={image}
            loading='lazy'
            alt={`Εικόνα παράδειγμα εκτύπωσης 3D ${index + 1}`}
          />
        ))}
      </div>
      <div className='imgBox-medium'>
        {images.slice(0, 6).map((image, index) => (
          <img
            ...
            src={image}
            loading='lazy'
            alt={`Εικόνα εκτύπωσης 3D ${index + 1}`}
          />
        ))}
      </div>
      <div className='imgBox-small'>
        {images.slice(0, 4).map((image, index) => (
          <img
            ...
            src={image}
            loading='lazy'
            alt={`Εκτύπωση 3D στην Κύπρο ${index + 1}`}
          />
        ))}
      </div>
    </div>
